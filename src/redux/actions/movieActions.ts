...
export const fetchNowPlayingMovies = (page = 1) => {
  debugger
  return <const>{
    type: action.FETCH_NOW_PLAYING_MOVIES,
    payload: { page },
  }
};
...
export const fetchNowPlayingMoviesSuccess = (data: IMovieResponse) => (<const>{
  type: action.FETCH_NOW_PLAYING_MOVIES_SUCCESS,
  payload: data
});
...
export type TMovieActionType =
  ...
  | ReturnType<typeof fetchNowPlayingMoviesSuccess>
